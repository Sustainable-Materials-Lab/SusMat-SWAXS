...
def cli():
    ...
    parser.add_argument(
        "--highscore", help="Output for highscore plus", action="store_true")
    parser.add_argument(
        "--exposure", help="Counting time (convert to counts from CPS)", type=float, default=1)
    # parser.add_argument("--dat", help="convert from dat file", action="store_true")
    parser.add_argument(
        "--SAXS", help="output SAXS data (q and don't cut low angle)", action="store_true")
    parser.add_argument(
        "--renormalize", help="boost counts", action="store_true")
    parser.add_argument("--clip", help="clip range", action="store_true")
    parser.add_argument(
        "--fullprof", help="general fullprof input", action="store_true")
    parser.add_argument(
        "--kratky", help="Generate Kratky plot columns", action="store_true")
    parser.add_argument("--pyfai", help="pyfai input", action="store_true")
    parser.add_argument(
        "--zero", help="set minimum to zero", action="store_true")
    ...
    if args.pyfai == True:
        ...
        data = np.genfromtxt(fname, skip_header=head,
                             usecols=(0, 1, 2), encoding="utf-8")
    ...
    np.savetxt(fname[:-4]+ext, out, delimiter=' ', fmt='%.10f',
               header=head2, comments='', newline='\n')
